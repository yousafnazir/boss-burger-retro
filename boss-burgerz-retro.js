(function () {
  ...
  function parsePrice(el) {
    const n = parseFloat((el?.textContent || '0').replace(/[^\d.]/g, ''));
    return Number.isFinite(n) ? n : 0;
  }
  function qtyTotal(items) { return items.reduce((s, i) => s + i.qty, 0); }
    ...
    document.body.insertAdjacentHTML('beforeend', `
      ...
      <aside class="drawer" id="cartDrawer" aria-hidden="true">
        <div class="drawer-head">
          ...
          <button class="stamp-btn icon-btn" id="closeCart" aria-label="Close cart">
  ...
  function renderCart() {
    ...
    if (!items.length) {
      ...
      list.innerHTML = items.map((i, idx) => `
        <div class="cart-row" data-idx="${idx}">
          <div><strong>${i.name}</strong><span> ${money(i.price)}</span></div>
          <div class="qty">
    ...
    const sub = $('#cartSub');
    if (sub) sub.textContent = money(subtotal(items));
    const bar = $('#mobileCart');
    const label = $('#mobileCartLabel');
    if (bar && label) {
      bar.hidden = count === 0;
      label.textContent = count + (count === 1 ? ' item' : ' items') + ' · ' + money(subtotal(items));
    }
    const mode = localStorage.getItem(MODE_KEY) || 'drive';
    const subEl = $('#drawerMode');
    if (subEl) subEl.textContent = (mode === 'walk' ? 'Walk-up window' : 'Drive-thru') + ' · ready in about 12 min';
  }
  ...
  $$('.add').forEach(btn => {
    btn.addEventListener('click', () => {
      const card = btn.closest('.hit-card, .food-card, .menu-card');
  ...
  $$('.fav').forEach(btn => {
    ...
    if (title && favs.includes(title)) btn.classList.add('is-on');
    btn.addEventListener('click', () => {
  ...
  $$('.mode-btn').forEach(btn => {
    ...
    btn.classList.toggle('is-on', btn.dataset.mode === saved);
    btn.addEventListener('click', () => {
  ...
  if (folderTabs.length) {
    ...
    $('.folder-tabs')?.addEventListener('keydown', (e) => {
  ...
  $$('.menu-card img, .hit-card img').forEach(img => {
